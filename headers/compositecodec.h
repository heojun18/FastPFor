...
public:
  ...
  void encodeArray(const uint32_t *in, const size_t length, uint32_t *out,
    ...
    codec1.encodeArray(in, roundedlength, out, nvalue1, skiplist);
  ...
  const uint32_t *decodeArray(const uint32_t *in, const size_t length,
    ...
    size_t mynvalue1 = nvalue;
		//std::cout << "[arcj] compositecode decodeArray() " << std::endl;
    //const uint32_t *in2 = codec1.decodeArray(in, length, out, mynvalue1);
    const uint32_t *in2 = codec1.decodeArray(in, length, out, mynvalue1, skiplist);
		//std::cout << "[arcj] compositecode in2 " << in2 << std::endl;
    if (length + in > in2) {
      ...
      const uint32_t *in3 = codec2.decodeArray(in2, length - (in2 - in),
                                               ...
                                               out + mynvalue1, nvalue2, skiplist);
